...
const AddressBasedTransactionForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        if (response.data.status === 'address_failure') {
            ...
            if (paymentSuccess) {
                try {
                    await axios.post('transactiontype-rcfpsxcera-uc.a.run.app/transaction_api/address-transfer/', {
                        transaction_amount: transactionAmount,
                        transaction_currency: transactionCurrency,
  ...
  const handleContinue = () => {
    router.push('/WalletTransactionInterface'); // Update this route as needed
  };
  const settinghandleBackClick = () => {
    router.push('/WalletTransactionInterface'); // Update this route as needed
  };
