...
const EmailForm = ({ onEmailSent }) => {
  ...
  const handleEmailSubmit = async () => {
    try {
      const response = await axios.post('https://notificationservice-ind-255574993735.asia-south1.run.app/emailverificationapi/verify-email/', { email_id: email });
...
//   const handleEmailSubmit = async () => {
//     try {
//       const response = await axios.post('https://notificationservice-ind-255574993735.asia-south1.run.app/api/verify-email/', { email_id: email });
//       setMessage(response.data.message);
//       onEmailSent(email);
//     } catch (error) {
//       setMessage(error.response?.data?.error || 'An error occurred');
