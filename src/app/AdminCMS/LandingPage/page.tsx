...
const Home = () => {
  ...
  const handleAddClick = async () => {
    ...
    if (showTextBox === "currency")
      formData.append("currency_type", inputValue);
    if (file) {
    ...
    try {
      const response = await fetch(`${AdminCMS}/api/admincms/`, {
        method: "POST",
      ...
      toast.error("An error occurred while adding the value.", {
        autoClose: false,
      });
    } finally {
      setIsLoading(false);
  ...
  const handleSave = async (id: number) => {
    ...
    try {
      const response = await fetch(`${AdminCMS}/api/admincms/${id}/`, {
        method: "PUT",
  ...
  const handleDelete = async (id: number) => {
    if (
      await toast.promise(
        fetch(`${AdminCMS}/api/admincms/${id}/`, {
          method: "DELETE",
        }).then((response) => {
          if (response.ok) {
      ...
      {isInputVisible && (
        <div className={styles.textBoxContainer}>
          <input
            type="text"
            placeholder={`Enter ${
              showTextBox === "account" ? "Account" : "Currency"
            } Type`}
            value={inputValue}
                      ...
                      {editMode === item.id ? (
                        ...
                        <>
                          <FaEdit
                            className={styles.icon}
                            onClick={() =>
                              handleEdit(item.id, item.account_type ?? "")
                            }
                          />
                          <FaTrash
                      ...
                      {editMode === item.id ? (
                        ...
                        <>
                          <FaEdit
                            className={styles.icon}
                            onClick={() =>
                              handleEdit(item.id, item.currency_type ?? "")
                            }
                          />
                          <FaTrash
                            className={styles.deleteIcon}
